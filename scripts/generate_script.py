"""
generate_script.py
يستخدم Grok API (xAI) لتوليد عنوان + وصف + تاجز + سكريبت مقسّم لمشاهد.
...
def main():
    ...
    if not api_key:
        print("خطأ: XAI_API_KEY غير موجود", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"خطأ: تعذر تفسير مخرجات Grok كـ JSON: {e}", file=sys.stderr)
